...
export function initState(){
  ...
  levelI.addEventListener('input', e => { state.level = Math.max(1, parseInt(e.target.value) || 1); clampPerksToSlots(); renderAll(); });
...
function renderSheet(){
  ...
  const perksEl = document.getElementById('sPerks');
  if(perksEl){
    ensurePerks();
    const names = state.choices.perks.map(pid => (PERKS.find(p => p.id === pid)?.name || pid));
    perksEl.textContent = names.length ? names.join(', ') : '—';
  }
}
...
export const PERKS = [
  // Combat
  { id:'gunslinger', name:'Gunslinger', category:'Combat', desc:'+2 to Guns with pistols. Pistol attacks cost 1 less AP (min 1).' },
  { id:'commando', name:'Commando', category:'Combat', desc:'+2 to Guns with rifles/automatics. Ignore 1 DR.' },
  { id:'shotgun_surgeon', name:'Shotgun Surgeon', category:'Combat', desc:'Shotguns ignore 2 DR on every hit.' },
  { id:'laser_commander', name:'Laser Commander', category:'Combat', desc:'+1 to hit and +1 damage with Energy Weapons.' },
  { id:'demolition_expert', name:'Demolition Expert', category:'Combat', desc:'Explosives deal +1 damage die on crit. Failed disarms never explode.' },
  { id:'super_slam', name:'Super Slam', category:'Combat', desc:'Crits with Melee/Unarmed force END TN 15 or target is knocked prone.' },
  { id:'slayer', name:'Slayer', category:'Combat', desc:'+1 damage with Melee and Unarmed. +1 AGI to initiative only.' },
  { id:'sniper', name:'Sniper', category:'Combat', desc:'Ranged attacks ignore penalties from half-cover at Near or Far.' },

  // Stealth & Mobility
  { id:'ninja', name:'Ninja', category:'Stealth & Mobility', desc:'+2 Sneak. +1d6 damage when striking from stealth with Melee/Unarmed.' },
  { id:'silent_running', name:'Silent Running', category:'Stealth & Mobility', desc:'Sneak suffers no penalty from movement, sprinting, or using pistols.' },
  { id:'thief', name:'Thief', category:'Stealth & Mobility', desc:'+1 Sneak/Lockpick. +2 to steal/pickpocket.' },
  { id:'action_boy', name:'Action Boy', category:'Stealth & Mobility', desc:'Once per encounter, regain all used AP this turn, to be used immediately.' },
  { id:'ghostwalker', name:'Ghostwalker', category:'Stealth & Mobility', desc:'While undetected, gain +2 to all AGI-based rolls.' },
  { id:'backstabber', name:'Backstabber', category:'Stealth & Mobility', desc:'Deal +2 damage when attacking an unaware target (any weapon).' },
  { id:'tunnel_rat', name:'Tunnel Rat', category:'Stealth & Mobility', desc:'Ignore cramped/confined penalties. Count as one size smaller for squeezing.' },
  { id:'chameleon_skin', name:'Chameleon Skin', category:'Stealth & Mobility', desc:'If you stay still for one round, enemies suffer -2 to detect or attack you.' },

  // Survival & Medicine
  { id:'hunter', name:'Hunter', category:'Survival & Medicine', desc:'+2 Survival vs animals/mutants. Gain trophy/crafting components on kills.' },
  { id:'rapid_metabolism', name:'Rapid Metabolism', category:'Survival & Medicine', desc:'Healing restores +2 HP (robots excluded).' },
  { id:'chemist', name:'Chemist', category:'Survival & Medicine', desc:'Chems last twice as long.' },
  { id:'purifier', name:'Purifier', category:'Survival & Medicine', desc:'Deal +1 damage die in melee vs Mutants/Ghouls/Beasts.' },
  { id:'toughness', name:'Toughness', category:'Survival & Medicine', desc:'Gain +1 DR permanently.' },
  { id:'field_medic', name:'Field Medic', category:'Survival & Medicine', desc:'Stabilize dying allies as a free action once per encounter, or half AP thereafter.' },
  { id:'green_thumb', name:'Green Thumb', category:'Survival & Medicine', desc:'Gather double harvest from plants, fungus, and edible materials.' },
  { id:'rad_sponge', name:'Rad Sponge', category:'Survival & Medicine', desc:'Reduce incoming RAD by 50% per source (min 1 total per scene).' },

  // Crafting & Technical
  { id:'jury_rigging', name:'Jury Rigging', category:'Crafting & Technical', desc:'Repair items using anything conceptually similar; requires creativity.' },
  { id:'scrapper', name:'Scrapper', category:'Crafting & Technical', desc:'Gain extra components when looting tech.' },
  { id:'weapon_artisan', name:'Weapon Artisan', category:'Crafting & Technical', desc:'One weapon permanently gains +1.' },
  { id:'computer_whiz', name:'Computer Whiz', category:'Crafting & Technical', desc:'Once per session, auto-succeed a Science terminal roll.' },
  { id:'robco_certified', name:'RobCo Certified', category:'Crafting & Technical', desc:'Robots treat you as neutral/friendly until harmed.' },
  { id:'tinker', name:'Tinker', category:'Crafting & Technical', desc:'Reduce repair costs by 50%. Ignore breakage on failed Repair rolls.' },
  { id:'modsmith', name:'Modsmith', category:'Crafting & Technical', desc:'Build mods without exotic parts; scavenge substitutes on success.' },
  { id:'emp_specialist', name:'EMP Specialist', category:'Crafting & Technical', desc:'+2 to attack robots or turrets. On a crit, shut one down for a round.' },

  // Social & Utility
  { id:'black_widow_lady_killer', name:'Black Widow / Lady Killer', category:'Social & Utility', desc:'+2 Speech against opposite sex. +1 to hit against them.' },
  { id:'inspirational', name:'Inspirational', category:'Social & Utility', desc:'Allies within Near gain +1 to next roll once per encounter.' },
  { id:'negotiator', name:'Negotiator', category:'Social & Utility', desc:'+1 Speech and +1 Barter. Once/session turn combat into negotiation if plausible.' },
  { id:'scavenger', name:'Scavenger', category:'Social & Utility', desc:'Loot yields more utility items (caps, ammo, food).' },
  { id:'pack_mule', name:'Pack Mule', category:'Social & Utility', desc:'Increase carry weight by 10 × STR for calculating encumbrance.' },
  { id:'smooth_talker', name:'Smooth Talker', category:'Social & Utility', desc:'Ignore one negative modifier to Speech checks.' },
  { id:'wasteland_scholar', name:'Wasteland Scholar', category:'Social & Utility', desc:'+1 Medicine and +1 Science. Recognize faction insignias and relics.' },
  { id:'bar_crawl', name:'Bar Crawl', category:'Social & Utility', desc:'Vendors offer better prices if you share a drink or favor.' },

  // Luck & Chaos
  { id:'fortune_finder', name:'Fortune Finder', category:'Luck & Chaos', desc:'Loot piles drop 2x-3x more caps.' },
  { id:'grim_reapers_sprint', name:'Grim Reaper’s Sprint', category:'Luck & Chaos', desc:'Once per encounter regain 2 AP when you down an enemy.' },
  { id:'bloody_mess', name:'Bloody Mess', category:'Luck & Chaos', desc:'Global +1 damage. Kills are explosively grisly.' },
  { id:'here_and_now', name:'Here and Now', category:'Luck & Chaos', desc:'Gain one Tag slot immediately.' },
  { id:'scrounger', name:'Scrounger', category:'Luck & Chaos', desc:'Ammo finds yield 50% more ammunition.' },
  { id:'luck_of_the_draw', name:'Luck of the Draw', category:'Luck & Chaos', desc:'Once per session turn a failed test into a success (not on crit fail).' },
  { id:'jam_proof', name:'Jam-Proof', category:'Luck & Chaos', desc:'Weapons do not jam or degrade on critical failures.' },
  { id:'coin_toss_killer', name:'Coin Toss Killer', category:'Luck & Chaos', desc:'After rolling damage, reroll and take the higher result once per encounter.' },

  // Wild Wasteland
  { id:'wild_wasteland', name:'Wild Wasteland', category:'Wild Wasteland', desc:'Optional party perk — experience the wackier side of the post apocalyptic world. Pop culture references, strange occurences, unique loot.' }
];
...
export function getPerkSlots(){
  const lvl = Number(state.level) || 1;
  // Level 1, 3, then every 3 levels thereafter (6,9,...,30)
  if(lvl < 1) return 0;
  let n = 0;
  if(lvl >= 1) n += 1;
  if(lvl >= 3) n += 1;
  if(lvl >= 6){
    n += Math.floor((Math.min(lvl, 30) - 6) / 3) + 1;
  }
  return n;
}

function clampPerksToSlots(){
  ensurePerks();
  const slots = getPerkSlots();
  if(state.choices.perks.length > slots){
    state.choices.perks = state.choices.perks.slice(0, slots);
  }
}

export function togglePerk(perkId){
  ensurePerks();
  const list = state.choices.perks;

  if(list.includes(perkId)){
    state.choices.perks = list.filter(p => p !== perkId);
    return;
  }
...
function getPerkCategories(){
  const set = new Set(PERKS.map(p => p.category));
...
function renderPerks(){
  ...
  const filter = document.getElementById('perkCategoryFilter');
  const slotsInfo = document.getElementById('perkSlotsInfo');

  // Populate filter once (and keep selection)
  if(filter && !filter.dataset.ready){
    const cur = filter.value || 'all';
    filter.innerHTML = '<option value="all">All</option>' + getPerkCategories()
  ...
  const slots = getPerkSlots();
  const used = state.choices.perks.length;
  if(slotsInfo) slotsInfo.textContent = `Selected: ${used}/${slots}`;

  const chosen = new Set(state.choices.perks);
  const cat = (filter && filter.value) ? filter.value : 'all';

  wrap.innerHTML = '';

  PERKS.filter(p => cat === 'all' || p.category === cat).forEach(p => {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'perk-card' + (chosen.has(p.id) ? ' active' : '');
    btn.innerHTML = `
      <div class="perk-name">${p.name}</div>
      <div class="perk-meta muted">${p.category}</div>
    ...
    // If locked because no slots left
    if(!chosen.has(p.id) && used >= slots){
      btn.disabled = true;
